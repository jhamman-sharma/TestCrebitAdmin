...
namespace Crebit_Admin
{
    public partial class DashBoardAdmin : System.Web.UI.Page
    {
        ...
        protected void GetTransactionData(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // CyberPlate Profit By Admin, EnterPrise , Personal
                ...
                thisCommand.Parameters.AddWithValue("@DATE", String.IsNullOrEmpty(DATE.Value) ? Convert.DBNull : Convert.ToDateTime(DATE.Value).Date);
                    ...
                    foreach (DataRow item in drcUserCharges)
                   {
                    ...
                    // Total Profit  COunt And Amount By Admin and  User 
                     ...
                     foreach (DataRow item in drcProfit)
                     {
                         ...
                         {
                         CP_EntCount +=1;
                         CP_EntProfit += float.Parse(UserProfit);
                              if (nowdate.Equals(date))
                                {
                                    Today_CP_EntCount += 1;
                                    Today_CP_EntProfit += float.Parse(UserProfit);
                                }
